...
function Entries({entries, setEntries}) {
    ...
    useEffect(() => {
        if (entries.length > 0) localStorage.setItem("entries", JSON.stringify(entries));
    }, [entries]);
    ...
    return (
        <div className = "entries-box">
            <div className = "title-plus">
                <h1>Entries</h1>
                <button className={`add-entry-toggle ${showForm ? "active-plus" : ""}`} onClick = {() => setShowForm(!showForm)}> {/* active-plus is used only if showForm is set to true */}
                    +
                </button>
            </div>
